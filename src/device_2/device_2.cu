...
void removeSingleSeam(uchar3 *inPixels, int width, int height, int seam_order, uchar3 *outPixels, int blocksize)
{
    ...
    blurImg_kernel_v2<<<gridSize_blur, blockSize>>>(d_src_gray, width, height, d_src_blur);

    // 3. Calc Energies
    CHECK(cudaMalloc(&d_energies, pixelsSize_1channels));

    calcEnergyMap_kernel_v2<<<gridSize_blur, blockSize>>>(d_src_blur, width, height, d_energies);
